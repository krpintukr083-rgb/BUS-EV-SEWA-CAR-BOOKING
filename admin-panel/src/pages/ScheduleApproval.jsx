...
export default function ScheduleApproval() {
  ...
  const load = async () => { try { const res = await adminService.getPendingSchedules(); setSchedules(res.data || []); setError(''); } catch (e) { setError(e?.response?.data?.message || 'Unable to load schedule approvals'); } };
  useEffect(() => { load(); }, []);
  ...
  return <div className="page-container">
    <div className="card-header-flex"><div><h1>Schedule Approval</h1><p style={{ color: '#64748b' }}>Review pending driver-submitted routes and departure schedules.</p><p style={{ color: '#64748b', fontSize: '.8rem' }}>Only pending schedules are available from the existing admin API; approved and rejected history is not exposed.</p></div></div>
    {error && <p className="error-message">{error}</p>}<div className="content-card"><div className="table-responsive"><table className="data-table"><thead><tr><th>Route</th><th>Date & departure</th><th>Vehicle</th><th>Driver</th><th>Status</th><th>Actions</th></tr></thead><tbody>
      {schedules.map(s => <tr key={s._id}><td><strong>{s.origin} → {s.destination}</strong><div style={{ color: '#64748b' }}>{s.stops?.length ? `${s.stops.length} stop(s)` : 'Direct route'}</div></td><td>{s.travelDate ? new Date(s.travelDate).toLocaleDateString() : '—'}<div style={{ color: '#64748b' }}>{s.departureTime || '—'}{s.arrivalTime ? ` – ${s.arrivalTime}` : ''}</div></td><td>{s.vehicle?.vehicleNumber || '—'}</td><td>{s.driver?.name || '—'}</td><td><StatusBadge status={s.status || 'Pending'} /></td><td><button className="btn btn-sm btn-outline" onClick={() => setSelected(s)}><Eye size={13} /> Details</button><button className="btn btn-sm btn-success" onClick={() => review(s, true)}><Check size={13} /> Approve</button><button className="btn btn-sm btn-danger" onClick={() => setRejecting(s)}>Reject</button></td></tr>)}
      {!schedules.length && <tr><td colSpan="6" style={{ textAlign: 'center', padding: 24, color: '#64748b' }}>No pending schedule approvals.</td></tr>}
    </tbody></table></div></div>
