...
export default function VehicleApproval() {
  ...
  return <div className="page-container">
    ...
    {selected && <div className="modal-overlay"><div className="modal-content" style={{ maxWidth: 720 }}><div className="card-header-flex"><h3>Vehicle details</h3><button className="btn btn-outline btn-sm" onClick={() => setSelected(null)}><X size={14} /></button></div><p><strong>{selected.vehicleNumber}</strong> · {selected.vehicleName || selected.vehicleModel} · {selected.vehicleType}</p><p>Owner: {selected.ownerName || '—'} · Driver: {selected.assignedDriver?.name || '—'} · Submitted: {selected.createdAt ? new Date(selected.createdAt).toLocaleString() : '—'}</p><p>Seats: {selected.seatingCapacity || selected.busDetails?.availableSeats || '—'} · Fuel: {selected.carDetails?.fuelType || '—'} · AC: {selected.carDetails ? (selected.carDetails.ac ? 'Yes' : 'No') : '—'}</p><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 16 }}>{(selected.vehicleImages || []).map((img, i) => <a key={i} href={resolveUrl(img)} target="_blank" rel="noreferrer"><img src={resolveUrl(img)} alt={`Vehicle ${i + 1}`} style={{ width: 150, height: 100, objectFit: 'cover', borderRadius: 6 }} /></a>)}</div><div style={{ marginTop: 16 }}>{['rcDocument', 'insuranceDocument', 'fitnessDocument', 'permitDocument'].map(key => selected[key] && <a key={key} className="btn btn-sm btn-outline" style={{ marginRight: 6 }} href={resolveUrl(selected[key])} target="_blank" rel="noreferrer"><FileText size={13} /> {key.replace('Document', '')}</a>)}</div></div></div>}
